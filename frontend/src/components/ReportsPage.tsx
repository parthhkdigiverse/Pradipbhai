...
export function ReportsPage({ setCurrentPage: _setCurrentPage }: { setCurrentPage?: (page: string) => void }) {
  const { clients, jobs, staff, products, vendors, invoices, currentUserRole, hasPermission } = useData();
  const canExport = hasPermission(currentUserRole, 'Export Reports');
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-white/30">
              ...
              {canExport && (
                <div className="flex gap-2">
                  <button className="px-3 py-1.5 bg-primary/10 text-primary hover:bg-primary/20 rounded-lg text-xs font-bold transition-colors flex items-center gap-1.5 cursor-pointer">
                    <Download className="w-3.5 h-3.5" />
                    Export to Excel
                  </button>
                  <button className="px-3 py-1.5 bg-indigo-600/10 text-indigo-700 hover:bg-indigo-600/20 rounded-lg text-xs font-bold transition-colors flex items-center gap-1.5 cursor-pointer">
                    <Download className="w-3.5 h-3.5" />
                    Export to PDF
                  </button>
                </div>
